...
describe("validate", () => {
  ...
  it("rejects missing required fields", () => {
    ...
    if (!result.valid) {
      expect(result.errors.some((e) => e.path.includes("session_id"))).toBe(true);
    }
  });
  ...
  it("rejects unknown event_type", () => {
    ...
    expect(result.valid).toBe(false);
    if (!result.valid) {
      expect(result.errors.some((e) => e.path.includes("event_type"))).toBe(true);
    }
  });
  ...
  it("rejects non-UUID machine_id", () => {
    ...
    expect(result.valid).toBe(false);
    if (!result.valid) {
      expect(result.errors.some((e) => e.path.includes("machine_id"))).toBe(true);
    }
  });
