...
const enumDuplicates = enumValues.filter(
  (v, i) => enumValues.indexOf(v) !== i,
);
if (enumDuplicates.length > 0) {
  error(`event_type enum has duplicates: ${[...new Set(enumDuplicates)].join(", ")}`);
}
...
for (const filename of readdirSync(PAYLOAD_DIR)) {
  ...
  for (const key of Object.keys(defs)) {
    if (!enumSet.has(key)) {
      error(`${filename} declares payload for "${key}" but it is not in the envelope event_type enum`);
    }
    if (definedTypes.has(key)) {
      error(`payload "${key}" is defined in multiple payload files (latest: ${filename})`);
    }
    definedTypes.add(key);
    const def = defs[key];
    if (def.additionalProperties !== false) {
      error(`${filename} $defs["${key}"] is missing "additionalProperties": false`);
    }
  }
}
